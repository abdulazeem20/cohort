...
try
{
	...
	shuffledQuestions.forEach((el, i) =>
	{
		...
		$("footer").append(`
                <button type="button" class="index ${index == 1 && "active"
...
{
	return $(`
        <button class="timer ${min < 5 && "timerUp"
		}" type="button"><span class="min">${min}</span>:<span class="sec">${String(sec).length == 1 ? "0" + sec : sec
		}</span></button>
    `);
}
